...
def request_html(ano: int, subopc: str) -> str:
    """
    Faz uma requisição HTTP para obter o HTML da página de importação.

    Args:
        ano (int): O ano para o qual os dados de importação serão buscados.
        subopc (str): A subopção específica para o tipo de dado de importação.

    Returns:
        str: O conteúdo HTML da página de importação.
...
def read_html_from_cache(ano: int, subopc: str) -> str:
    """
    Lê o HTML de importação de um arquivo cacheado.
    ...
    """
    path        = SAVING_PATH_IMPORTACAO + f"/{subopc}/"
...
def scraping_html(html: str) -> dict:
    """
    Extrai os dados estruturados do HTML de importação.

    Args:
        html (str): O conteúdo HTML contendo os dados de importação.
